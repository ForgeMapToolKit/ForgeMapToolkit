...
import './AddCustomPropOverlay.css';
...
function DropZone({ label, accept, icon, file, onFile, hint, optional }) {
  ...
  return (
    <div
      className={`acpo-dropzone${dragging ? ' dragging' : ''}${file ? (isValid ? ' has-file' : ' has-file invalid') : ''}${optional ? ' optional' : ''}`}
      onDrop={handleDrop}
    ...
    >
      ...
      <div className="acpo-dropzone-icon">{file ? (isValid ? '' : '') : icon}</div>
      <div className="acpo-dropzone-label">
        {label}
        {optional && <span className="acpo-optional-badge">optional</span>}
      </div>
      {file
        ? <div className="acpo-dropzone-filename">{file.name}</div>
        : <div className="acpo-dropzone-hint">{hint || 'Drop or click'}</div>}
    </div>
  );
}
...
function FolderDropZone({ folderName, fileList, onFolder, status, propCount }) {
  ...
  return (
    <div
      className={`acpo-folder-dropzone${dragging ? ' dragging' : ''}${folderName ? ' has-folder' : ''}${status === 'ok' ? ' folder-ok' : ''}`}
      onDrop={handleDrop}
      onDragOver={e => { e.preventDefault(); setDragging(true); }}
      onDragLeave={() => setDragging(false)}
      onClick={() => inputRef.current?.click()}
    >
      ...
      {folderName ? (
        <>
          <div className="acpo-folder-icon"></div>
          ...
          <div className="acpo-folder-reclick-hint">Click or drop again to change</div>
        </>
      ) : (
        <>
          <div className="acpo-folder-big-icon"></div>
          <div className="acpo-folder-drop-label">Drop prop folder here</div>
          <div className="acpo-folder-drop-hint">
            Folder containing <code>_prop.bp</code>, <code>.scm</code> and <code>.dds</code> files
          </div>
          <div className="acpo-folder-drop-hint" style={{ marginTop: 6, opacity: 0.5 }}>
            or click to browse
          </div>
        </>
      )}
...
function NumField({ label, value, onChange, min, step = 0.01 }) {
  return (
    <div className="acpo-field">
      <label className="acpo-field-label">{label}</label>
      <input type="number" className="acpo-field-input"
...
// ── Main overlay ──────────────────────────────────────────────────────────────
export default function AddCustomPropOverlay({ onConfirm, onSaved, onClose }) {
  // 'manual' | 'folder'
      ...
      <div className="acpo-panel">
        ...
        <div className="acpo-body">
          ...
          <div className="acpo-section">
            ...
            <div className="acpo-type-switcher">
              <button className={`acpo-type-btn${inputMode === 'manual' ? ' active' : ''}`}
                onClick={() => setInputMode('manual')}>
                 Manual
              </button>
              <button className={`acpo-type-btn${inputMode === 'folder' ? ' active' : ''}`}
                ...
                return (
                  <div className="acpo-section">
                    <label className="acpo-section-label" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      {hasEdits && (
                        <button className="acpo-reset-edits-btn" onClick={() => resetPropBpEdits(prop.name)} title="Reset changes">
                          ↺ Reset
                        </button>
                      )}
                    </label>
                    ...
                    <div className="acpo-field acpo-field-full" style={{ marginTop: 8 }}>
                      ...
                      <input className="acpo-field-input" value={vals.helpText ?? ''}
                        onChange={e => setPropBpField(prop.name, 'helpText', e.target.value)}
                        placeholder={prop.name} />
                    </div>
                    <div className="acpo-field acpo-field-checkbox" style={{ marginTop: 8 }}>
                      <label className="acpo-field-label">Block Pathfinding</label>
                      <button className={`acpo-toggle${vals.blockPath ? ' on' : ' off'}`}
                        onClick={() => setPropBpField(prop.name, 'blockPath', !vals.blockPath)}>
                        {vals.blockPath ? 'YES' : 'NO'}
                      </button>
                    </div>
                    {vals._raw && (
          ...
          {inputMode === 'manual' && (
            <>
              ...
              <div className="acpo-section">
                ...
                <div className="acpo-type-switcher">
                  <button className={`acpo-type-btn${propType === 'tree' ? ' active' : ''}`} onClick={() => switchType('tree')}> Tree</button>
                  <button className={`acpo-type-btn${propType === 'rock' ? ' active' : ''}`} onClick={() => switchType('rock')}> Rock</button>
                </div>
              </div>
              ...
              <div className="acpo-section">
                ...
                <input className="acpo-name-input" value={name}
              ...
              <div className="acpo-section">
                ...
                <div className="acpo-dropzones">
                  <DropZone label="Mesh LOD0" icon="" accept={['.scm']} file={scmLod0} onFile={setScmLod0} hint=".scm (required)" />
                  <DropZone label="Albedo" icon="" accept={['.dds']} file={albedoFile} onFile={setAlbedoFile} hint=".dds only" />
                  <DropZone label="Normal Map" icon="" accept={['.dds']} file={normalFile} onFile={setNormalFile} hint=".dds only" />
                </div>
                {scmLod0 && (
                  <div className="acpo-lod-extra">
                    {!showExtraLods ? (
                      <button className="acpo-lod-add-btn" onClick={() => setShowExtraLods(true)}>+ Add LOD levels (LOD1 / LOD2)</button>
                    ) : (
                      <div className="acpo-lod-extra-grid">
                        <DropZone label="Mesh LOD1" icon="" accept={['.scm']} file={scmLod1} onFile={setScmLod1} hint=".scm" optional />
                        <DropZone label="Mesh LOD2" icon="" accept={['.scm']} file={scmLod2} onFile={setScmLod2} hint=".scm" optional />
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="acpo-section">
                ...
                <div className="acpo-bp-tabs">
                  <button className={`acpo-bp-tab${bpMode === 'upload' ? ' active' : ''}`} onClick={() => setBpMode('upload')}>Upload prop.bp</button>
                  <button className={`acpo-bp-tab${bpMode === 'create' ? ' active' : ''}`} onClick={() => setBpMode('create')}>Create prop.bp</button>
                </div>
                {bpMode === 'upload' && (
                  <div className="acpo-bp-upload">
                    <DropZone label="Blueprint" icon="" accept={['.bp']} file={bpFile} onFile={setBpFile} hint="Drop _prop.bp here" />
                  </div>
                )}
                {bpMode === 'create' && (
                  <div className="acpo-bp-fields">
                    ...
                    <div className="acpo-field acpo-field-full">
                      <label className="acpo-field-label">Help Text</label>
                      <input className="acpo-field-input" value={bp.helpText}
                    ...
                    <div className="acpo-field acpo-field-checkbox">
                      <label className="acpo-field-label">Block Pathfinding</label>
                      <button className={`acpo-toggle${bp.blockPath ? ' on' : ' off'}`} onClick={() => setBpField('blockPath', !bp.blockPath)}>
                        {bp.blockPath ? 'YES' : 'NO'}
                      </button>
                    </div>
                    {propName && (
        ...
        <div className="acpo-footer">
          ...
          <div className="acpo-footer-actions">
            <button className="acpo-btn-cancel" onClick={onClose}>Cancel</button>
            <button className={`acpo-btn-save${canSave ? ' ready' : ''}`} onClick={handleSave} disabled={!canSave || saving}>
