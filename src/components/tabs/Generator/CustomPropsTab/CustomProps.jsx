...
import AddCustomPropOverlay from './AddCustomProps/AddCustomPropOverlay';
...
import { adjHasChanges, DEFAULT_ADJUSTMENTS } from './TextureAdjustments/TextureAdjustOverlay';
import PropTextureEditor from './TextureAdjustments/Editor/PropTextureEditor';
...
export default function CustomPropsTab({ settings, shared = {}, onSharedChange = () => {} }) {
  ...
  return (
    <div className="cpt-tab">
      {/* ── Help button — only visible on the main Custom Props tab, not inside Library/TextureEditor ── */}
      {!showLibrary && !showTextureEditor && !showAddCustomProp && (
        <button
          className="help-btn"
      ...
      {showAddCustomProp && (
        <AddCustomPropOverlay
          ...
          onClose={() => setShowAddCustomProp(false)}
        />
      )}
      ...
      {showTextureEditor && customProps.length > 0 && (
        <PropTextureEditor
          selectedProps={customProps.map(entry => ({
            ...entry.originalProp,
            // Merge fresh lodPreviewUrls resolved at open-time (overrides frozen snapshot)
      ...
      <div className="cpt-content-grid">
        ...
        <div className="cpt-config-column">
          ...
          <div className="cpt-section-card">
            ...
            <div className="cpt-form-group">
              <label className="cpt-form-label">Map Name</label>
              <input
                type="text"
                className="cpt-form-input"
                value={mapName}
            ...
            {/* Add Custom Prop button — inside configuration card */}
            <button
              className="cpt-btn-add-custom-prop"
              onClick={() => setShowAddCustomProp(true)}
            >
          ...
          <div className="cpt-section-card">
            <h2 className="cpt-section-title">
              ...
              {customProps.length > 0 && (
                <span className="cpt-title-badge">{customProps.length} · {opsCount} with textures</span>
              )}
            </h2>
            ...
            <div className="cpt-props-grid">
              {customProps.map((entry, idx) => {
                ...
                // Link sources: other props with adjustments that are not themselves linked
return (
                  <div
                    key={entry.id}
              ...
              {/* Add card */}
              <div className="cpt-add-prop-card" onClick={() => setShowLibrary(true)}>
          ...
          <div className="section-card">
            <label className="checkbox-label" onClick={() => setGenerateReadme(v => !v)}>
            ...
            <button
              className="cpt-btn-primary cpt-btn-large"
        ...
        <div className="cpt-detail-column">
          <div className="cpt-section-card cpt-detail-outer">
            {!selected ? (
