...
import UnitLibraryOverlay from '../../../Shared/Libraries/UnitLibrary/UnitLibrary.jsx';
import EmitterLibraryOverlay from '../../../Shared/Libraries/EmitterLibrary/EmitterLibrary.jsx';
import { WreckageHelp, WreckageHelpButton } from './Help.jsx';
import TabLayout from '../../../Shared/Ui/TabLayout/TabLayout';
import {
  MapPreview, MirrorDropdown,
...
import WreckageConfiguration from './Configuration.jsx';
import WreckageUnits from './Units.jsx';
import WreckageExport from './Export.jsx';

// -- Defaults ------------------------------------------------------------------

const DEFAULT_UNIT = {
  id: Date.now(),
  unitType: '', unitName: '', unitEmitters: [], emitterSourceId: null, isEmitterSource: false,
...
const WreckageTab = ({ settings, shared = {}, onSharedChange = () => {}, onRecordSnapshot = () => {} }) => {
  ...
  const [legendCollapsed,     setLegendCollapsed]     = usePersistentState(s, 'wr_legendCollapsed', false, onSharedChange);
...
const addUnit = () => {
  const newUnit = {
    id: Date.now(),
    unitType: '',
    unitEmitters: [], emitterSourceId: null, isEmitterSource: false,
    unitName: '',
    coordinates: [{ x: '', y: '', z: '', heading: 'math.pi', pitch: '0.0', roll: 'math.pi', isMirrored: false, mirrorPairId: null }],
    color: `hsl(${Math.random() * 360}, 70%, 60%)`
  };
...
// -- Direct Emitter Management per Unit ---------------------------------------
...
const getEmittersForUnit = (unit) => {
  const valid = blueprintPaths.filter(p => p.trim());
  if (!valid.length) return [];
  const sourceUnit = unit.emitterSourceId != null
    ? units.find(u => u.id === unit.emitterSourceId)
    : null;
  const active = sourceUnit
    ? (sourceUnit.unitEmitters || [])
    : (unit.unitEmitters || []);
  const result = active.filter(p => valid.includes(p));
  return result.length > 0 ? result : valid;
};

const toggleUnitEmitter = (unitIndex, path) => {
  setUnits(prev => prev.map((u, i) => {
    if (i !== unitIndex) return u;
    const current = u.unitEmitters || [];
    const next = current.includes(path)
      ? current.filter(p => p !== path)
      : [...current, path];
    return { ...u, unitEmitters: next };
  }));
};

const setUnitSource = (unitIndex, sourceId) => {
  setUnits(prev => prev.map((u, i) => i === unitIndex ? { ...u, emitterSourceId: sourceId } : u));
};

const clearUnitSource = (unitIndex) => {
  setUnits(prev => prev.map((u, i) => i === unitIndex ? { ...u, emitterSourceId: null } : u));
};

// Declare / un-declare a card as an emitter source. A source defines its own
// selection, so becoming a source clears any inherited link. Un-declaring a
// source detaches every follower so nothing inherits from a non-source.
const setUnitIsSource = (unitIndex, isSrc) => {
  setUnits(prev => {
    const targetId = prev[unitIndex]?.id;
    return prev.map((u, i) => {
      if (i === unitIndex) {
        return { ...u, isEmitterSource: isSrc, emitterSourceId: isSrc ? null : u.emitterSourceId };
      }
      if (!isSrc && u.emitterSourceId === targetId) {
        return { ...u, emitterSourceId: null };
      }
      return u;
    });
  });
...
const handleImageUpload = async (e) => {
  ...
  const generateFiles = async () => {
    try {
      ...
      let emittersFailed = [];
      const propsLuaEntries = []; // { path, x, z } f�r SCMAP props.lua
...
`    {
  ...
  const handleImportFromMap = async () => {
    ...
    try {
      ...
      // -- 2. _save.lua lesen � UNIT_XXXX Bl�cke ----------------------------
      ...
      setUnits(prev => {
        ...
        for (const unitType of unitTypes) {
          ...
          if (existingIdx !== -1) {
            // Karte existiert bereits � nur neue Koordinaten anf�gen
  ...
  const handleCanvasLeave = () => {
    if (coordReadoutRef.current) coordReadoutRef.current.textContent = 'X  -  Z';
  };
  ...
  const unitsProps = {
    ...
    blueprintPaths,
    getEmitterName,
    onToggleUnitEmitter: toggleUnitEmitter,
    onSetUnitSource: setUnitSource,
    onClearUnitSource: clearUnitSource,
    onSetUnitIsSource: setUnitIsSource,
  };
  ...
  const previewSlot = (
    <MapPreview
      ...
      selectedLegendId={units[selectedUnit]?.id}
      onLegendSelect={(id, idx) => setSelectedUnit(idx)}
