...
import { getMirroredCoords, kmLabel, ensureDir, writeFile, injectPropsLua, resolveToolkitEmitterPublicPaths, drawPlacementCanvas, usePersistentState, useMapInfo, useScmapPreview, } from '../../../Shared/MapLogic';
import PropsConfiguration from './Configuration.jsx';
import PropsPropList from './PropList.jsx';
import PropsExport from './Export.jsx';
...
const PropsTab = ({ settings, shared = {}, onSharedChange = () => {}, onRecordSnapshot = () => {} }) => {
  ...
  const [legendCollapsed,     setLegendCollapsed]     = usePersistentState(s, 'pt_legendCollapsed', false, onSharedChange);
...
const getEmittersForProp = (prop) => {
  ...
  const sourceProp = prop.emitterSourceId != null
    ? props.find(p => p.id === prop.emitterSourceId)
    : null;
  const active = sourceProp
    ? (sourceProp.unitEmitters || [])
    : (prop.unitEmitters || []);
  const result = active.filter(p => valid.includes(p));
  return result.length > 0 ? result : valid;
};

const toggleUnitEmitter = (propIndex, path) => {
  setProps(prev => prev.map((p, i) => {
    if (i !== propIndex) return p;
    const current = p.unitEmitters || [];
    const next = current.includes(path)
      ? current.filter(x => x !== path)
      : [...current, path];
    return { ...p, unitEmitters: next };
  }));
};

const setUnitSource = (propIndex, sourceId) => {
  setProps(prev => prev.map((p, i) => i === propIndex ? { ...p, emitterSourceId: sourceId } : p));
};

const clearUnitSource = (propIndex) => {
  setProps(prev => prev.map((p, i) => i === propIndex ? { ...p, emitterSourceId: null } : p));
};
...
const setUnitIsSource = (propIndex, isSrc) => {
  setProps(prev => {
    const targetId = prev[propIndex]?.id;
    return prev.map((p, i) => {
      if (i === propIndex) {
        return { ...p, isEmitterSource: isSrc, emitterSourceId: isSrc ? null : p.emitterSourceId };
      }
      if (!isSrc && p.emitterSourceId === targetId) {
        return { ...p, emitterSourceId: null };
      }
      return p;
    });
  });
...
`    {
  ...
  return (
    <div className="props-tab trace-tab">
      {/* ── Props Import Modal ── */}
  ...
  />
      ...
      <TabLayout
        sections={[
          ...
          { id: 'export',   index: '03', label: 'Export',        desc: 'Generate prop marker files and inject them into the .scmap.' },
        ]}
        ...
        asideSlot={
          <MapPreview
            ...
            selectedLegendId={props[selectedProp]?.id}
