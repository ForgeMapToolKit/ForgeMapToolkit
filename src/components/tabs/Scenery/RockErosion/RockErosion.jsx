...
const allCounts = [
  ...
  return (
    <div className="rockerosion-tab">
      ...
      <TabLayout
        sections={[
          { id: 'setup', index: '01', label: 'Setup', desc: 'Configure map target, density parameters, grid resolution, size blending and randomize.' },
          { id: 'rocks', index: '02', label: 'Rocks', desc: 'Define rock cards by type (Debris, Scree, Erosion) and generate.' },
        ]}
        activeSection={activeSection}
        onSelect={setActiveSection}
        ghostLabel="ROCK EROSION"
        railStorageKey="rockerosion-rail-pinned"
        navLabel="Rock Erosion navigation"
        previewCaption=""
        mirrorSlot={
        ...
        previewSlot={
          <div className="tm-preview-body">
            <div className="tm-upload-row">
              <button className="action-button" style={{ flex: 1 }} onClick={() => previewUploadRef.current?.click()}>
                {previewLoading ? 'Loading…' : 'Upload Map Image'}
              </button>
              {previewImage && (
                <button className="action-button action-button--danger" onClick={deletePreview}>Clear</button>
              )}
            ...
            <div className="tm-canvas-wrap">
              ...
              {!previewImage && rockMarkers.length === 0 && (
                <div className="tm-canvas-empty">Upload erosion maps to generate rocks</div>
              )}
            </div>
      ...
      >
        ...
        {activeSection === 'setup' && (
          <>
            ...
            <div className="subsection-head">
              <span className="subsection-head-title">GRID RESOLUTION</span>
            </div>
            <div className="form-group">
              <label className="field-label">
                Grid Resolution
                <span style={{ float: 'right', color: 'var(--tab-color)', fontSize: '0.78rem' }}>
            ...
            <div className="subsection-head">
              <span className="subsection-head-title">SIZE BLENDING</span>
            </div>
            <div className="form-group">
              <label className="field-label">Size Blend Sigma (1–150)</label>
            ...
            <button onClick={handleRandomize} className="action-button action-button--full" style={{ marginTop: '8px' }}>
              Randomize
            </button>
          </>
        )}
        {activeSection === 'rocks' && (
          ...
          {rockTypeTab === 'debris' && (
            <div className="re-tab-section">
              <div className="subsection-head">
                <span className="subsection-head-title">DEBRIS CARDS</span>
                {debrisCards.length >= 1 && (
                  <button className="action-button action-button--danger" style={{ fontSize: '0.78rem' }} onClick={deleteAllDebrisCards} title="Delete all debris cards">
                    Delete All
                  </button>
                )}
              </div>
              {debrisCards.map((card, idx) => (
                <div key={card.id} className={`item-card${selectedDebrisCard === idx ? ' selected' : ''}`}
                  onClick={() => setSelectedDebrisCard(idx)}>
                  <div className="item-card-header">
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1 }}>
                      <div className="item-card-color-indicator" style={{ backgroundColor: card.color }}
                    ...
                    <div className="item-card-color-picker" onClick={(e) => e.stopPropagation()}>
                      <div className="item-card-color-grid">
                        {availableColors.map((color, colorIdx) => (
                          <div key={colorIdx} className="item-card-color-option"
                            style={{ backgroundColor: color }}
                            onClick={() => { updateCard('debris', idx, 'color', color); setShowRockColorPicker(null); }} />
                        ))}
                      </div>
                      <input type="text" className="field-input" placeholder="Custom color (#HEX or hsl())"
                  ...
                  {selectedDebrisCard === idx && (
                    <div className="item-card-details" onClick={(e) => e.stopPropagation()}>
                      <div className="form-group">
                        <label className="field-label">Debris Map</label>
                        ...
                        {card.spawnMapImage ? (
                          ...
                          <button className="action-button action-button--full"
                        ...
                        {card.blueprintPaths.map((path, pi) => (
                          <div key={pi} className="input-row">
                            <input value={path} onChange={(e) => updateBlueprintPath('debris', idx, pi, e.target.value)}
                              placeholder="/env/Evergreen/Props/Rocks/Rock01_prop.bp" className="field-input"
                      ...
                      <div className="form-group" style={{ marginBottom: 0 }}>
                        ...
                        {card.useCustomValues && (
                          <div className="re-custom-values-body" onClick={(e) => e.stopPropagation()}>
                            <label className="field-label">Density Multiplier</label>
              ...
              <div className="item-card-add" onClick={() => addCard('debris')}>
                <div className="item-card-add-icon"><span style={{ fontSize: '2rem' }}>+</span></div>
                <span className="item-card-add-text">ADD DEBRIS CARD</span>
              </div>
            </div>
          )}
          ...
          {rockTypeTab === 'scree' && (
            <div className="re-tab-section">
              <div className="subsection-head">
                <span className="subsection-head-title">SCREE CARDS</span>
                {screeCards.length >= 1 && (
                  <button className="action-button action-button--danger" style={{ fontSize: '0.78rem' }} onClick={deleteAllScreeCards} title="Delete all scree cards">
                    Delete All
                  </button>
                )}
              </div>
              {screeCards.map((card, idx) => (
                <div key={card.id} className={`item-card${selectedScreeCard === idx ? ' selected' : ''}`}
                  ...
                  {selectedScreeCard === idx && (
                    <div className="item-card-details" onClick={(e) => e.stopPropagation()}>
                      <div className="form-group">
                        <label className="field-label">Scree Map</label>
              ...
              <div className="item-card-add" onClick={() => addCard('scree')}>
                <div className="item-card-add-icon"><span style={{ fontSize: '2rem' }}>+</span></div>
                <span className="item-card-add-text">ADD SCREE CARD</span>
              </div>
            </div>
          )}
          ...
          {rockTypeTab === 'erosion' && (<>
            <div className="re-tab-section">
              <div className="subsection-head">
                <span className="subsection-head-title">EROSION MAPS</span>
              </div>
              <p className="re-section-description">Upload masks to control gravitational rock distribution.</p>

              <div className="re-grid-2">
                <div>
                  <label className="field-label">Where to spawn?</label>
                  ...
                  {flowMask ? (
                    ...
                    <button className="action-button action-button--full" onClick={() => flowMaskInputRef.current?.click()}>Upload Spawn Map</button>
                  )}
                </div>
                <div>
                  <label className="field-label">Spawn probability</label>
                  ...
                  {slopeMask ? (
                    ...
                    <button className="action-button action-button--full" onClick={() => slopeMaskInputRef.current?.click()}>Upload Spawn Probability</button>
                  )}
                </div>
              </div>
              ...
              <div className="re-grid-2" style={{ marginTop: '15px' }}>
                <div>
                  <label className="field-label">Size determination</label>
                  ...
                  {erosionWearMap ? (
                    ...
                    <button className="action-button action-button--full" onClick={() => erosionWearInputRef.current?.click()}>Upload Size Determination</button>
                  )}
                </div>
                <div>
                  <label className="field-label" style={{ opacity: 0.8 }}>Clustering (Optional)</label>
                  ...
                  {curvatureMap ? (
                    ...
                    <button className="action-button action-button--full" style={{ opacity: 0.8 }}
              ...
              <div style={{ marginTop: '15px' }}>
                <label className="field-label" style={{ opacity: 0.8 }}>
            ...
            <div className="re-tab-section">
              <div className="subsection-head">
                <span className="subsection-head-title">ROCK CONFIGURATION</span>
                {rockCards.length >= 1 && (
                  <button className="action-button action-button--danger" style={{ fontSize: '0.78rem' }} onClick={deleteAllRockCards} title="Delete all rock configuration">
                    Delete All
                  </button>
                )}
              </div>
              {rockCards.map((card, idx) => (
                <div key={card.id} className={`item-card${selectedRock === idx ? ' selected' : ''}`}
              ...
              <div className="item-card-add" onClick={addRockCard}>
                <div className="item-card-add-icon"><span style={{ fontSize: '2rem' }}>+</span></div>
                <span className="item-card-add-text">ADD ROCK CARD</span>
              </div>
            </div>
          </>)}
            ...
            <label className="checkbox-label" style={{ marginBottom: '10px' }} onClick={() => setGenerateReadme(v => !v)}>
            ...
            {isGenerating ? (
              ...
              <button className="commit-button" onClick={generateFiles}>
                ...
                <span className="commit-button-status">Rock Erosion</span>
