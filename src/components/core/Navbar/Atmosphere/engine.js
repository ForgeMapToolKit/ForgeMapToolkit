...
export function createAtmosphereEngine(mount, { quality = 'fast', layer = 'back', rightBias = 0, diagShift = 0 } = {}) {
  ...
  let dprCap = quality === 'performant' ? 1.0 : 1; // matched to navbar for consistent sharpness
  ...
  return {
    ...
    setQuality(q) {
      dprCap = q === 'performant' ? 1.0 : 1; // matched to navbar
      resize();
    },
